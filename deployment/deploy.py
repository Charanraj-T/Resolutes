...
flags.DEFINE_string("user_id", "1234", "User ID for session operations.")
...
flags.DEFINE_string("display_name", None, "Display name for the deployment.")
flags.DEFINE_string("custom_env", None, "Custom environment variables in key=value,key2=value2 format.")
flags.DEFINE_string("agent_folder", None, "Agent folder to use for deployment.")
...
def create() -> None:
    ...
    agent_folder = FLAGS.agent_folder if FLAGS.agent_folder else os.getenv("AGENT_FOLDER")
    if not agent_folder or not os.path.isdir(agent_folder):
        print(f"Error: Agent folder '{agent_folder}' does not exist.")
        return
    print(f"Loading agent from {agent_folder}...")
    root_agent = get_agent(agent_folder)
    requirements_path = os.path.join(agent_folder, "requirements.txt")
    requirements = []
    if os.path.exists(requirements_path):
        with open(requirements_path) as f:
            requirements = [line.strip() for line in f if line.strip() and not line.startswith('#')]
    else:
        print(f"Warning: {requirements_path} not found. Using default requirements.")
        requirements = [
    ...
    if FLAGS.display_name:
        ...
        display_name = f"{agent_folder}-agent-{timestamp}"
    ...
    env_vars = {}
    if FLAGS.custom_env:
        for pair in FLAGS.custom_env.split(","):
            if "=" in pair:
